...
def get_all_zgloszenia(
    ...
    data_od: Optional[date] = Query(None, description="Filtruj zgłoszenia od daty (YYYY-MM-DD)"),
    data_do: Optional[date] = Query(None, description="Filtruj zgłoszenia do daty (YYYY-MM-DD)"),
    db: Session = Depends(get_db)
):
    """
    ...
    - **search**: wyszukaj w opisie problemu lub adresie
    - **data_od**, **data_do**: filtruj zakres dat zgłoszeń
    """
    zgloszenia = ZgloszenieService.get_all_zgloszenia(
        ...
        search=search,
        date_from=datetime.combine(data_od, datetime.min.time()) if data_od else None,
        date_to=datetime.combine(data_do, datetime.max.time()) if data_do else None,
    )
    return zgloszenia
