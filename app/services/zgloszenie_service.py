...
class ZgloszenieService:
    ...
    def get_all_zgloszenia(
        ...
        if search:
            pattern = f"%{search}%"
            query = query.filter(
                Zgloszenie.problem.ilike(pattern) | Zgloszenie.adres.ilike(pattern)
            )
        ...
        if date_from:
            ...
            query = query.filter(Zgloszenie.data_zgloszenia >= start_dt)
        ...
        if date_to:
            ...
            query = query.filter(Zgloszenie.data_zgloszenia <= end_dt)
    ...
    def get_statistics(db: Session) -> dict:
        ...
        by_type = (
            db.query(
                ...
                func.count(Zgloszenie.id)
            )
            .group_by(Zgloszenie.typ_zgloszenia_id)
            .all()
        )
